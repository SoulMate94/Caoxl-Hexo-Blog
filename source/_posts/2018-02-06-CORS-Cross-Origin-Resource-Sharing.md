...
date: 2018-02-06 10:33:08
categories: Internet
tags: [CORS, PHP, Internet]
---
...
如果想拿到其他字段，就必须在`Access-Control-Expose-Headers里面指定。上面的例子指定，`getResponseHeader('FooBar')`可以返回FooBar字段的值
...
上面的HTTP回应中，关键的是`Access-Control-Allow-Origin`字段，表示`http://api.bob.com`可以请求数据。该字段也可以设为`星号(*)`，表示同意任意跨源请求。
